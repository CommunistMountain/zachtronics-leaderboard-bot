...
internal object OmQL {
    internal sealed interface QueryElement {
        ...
        class Pareto(val metrics: List<OmMetric<*>>) : QueryElement {
            override fun filter(records: Collection<OmMemoryRecord>): List<OmMemoryRecord> {
    ...
    internal fun parseQuery(query: String, possibleMetrics: List<OmMetric<*>>): List<QueryElement> {
        ...
        if (elements.none { qe ->
                qe is QueryElement.Min && OmMetric.OVERLAP in qe.metric.scoreParts ||
                        qe is QueryElement.Constraint && OmMetric.OVERLAP in qe.metric.scoreParts ||
                        qe is QueryElement.Pareto && OmMetric.OVERLAP in qe.metrics.flatMap { it.scoreParts }
            }) {
    ...
    private fun parseMetric(
        ...
        if (query.startsWith("false", ignoreCase = true)) {
            return OmMetric.Constant("", false) to "false".length
        }
        if (query.startsWith("true", ignoreCase = true)) {
            return OmMetric.Constant("", true) to "true".length
        }
        if (query[0].isLetter()) {
            val metric = parseBaseMetric(query, possibleMetrics)
            return metric to metric.displayName.length
        }
        val match = Regex("^[-+\\d]+").find(query)
        if (match != null) {
            return OmMetric.Constant("", match.value.toDouble()) to match.range.last + 1
        }
        throw IllegalArgumentException("Unknown metric(s): $query")
    }

    private fun <T : OmMetric<*>> parseBaseMetric(query: String, possibleMetrics: List<T>): T {
        return possibleMetrics.firstOrNull { query.startsWith(it.displayName, ignoreCase = true) }
            ?: throw IllegalArgumentException("Unknown metric(s): $query")
    }
